...
sct_unix('sct_register_multimodal -i src_reslice.nii -d dest_RPI.nii -param step=1,algo=translation,metric=MeanSquares,slicewise=1:step=2,slicewise=1,algo=affine:step=3,algo=bsplinesyn,slicewise=1:step=4,algo=syn,gradStep=0.2,smooth=0.1,metric=CC')
sct_unix('sct_concat_transfo -d dest_RPI.nii -w step0/step00GenericAffine.mat,step0/step01Warp.nii.gz,warp_src_reslice2dest_RPI.nii.gz -o warp_forward.nii.gz')
sct_unix('sct_concat_transfo -w warp_dest_RPI2src_reslice.nii.gz,step0/step01InverseWarp.nii.gz,-step0/step00GenericAffine.mat -d src.nii -o warp_inverse.nii.gz');
