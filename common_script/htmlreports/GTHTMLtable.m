...
NORMCOL =0;     % flag to indicate if colormaping is normalized per lines/columns or not
...
while (count<nargin),
  ...
  elseif (ischar(varargin{count}) && strcmpi(varargin{count},'colormap') && count>1 && count<nargin),
    ...
    count = count + 1;
    if (count<nargin) && (isnumeric(varargin{count+1}))
        NORMCOL = varargin{count+1};
    end
...
if isnumeric(MAT),
    if NORMCOL==1 % normalize columns
        MATmax = max(MAT,[],1);
        MATmin = min(MAT,[],1);
        MATrange = MATmax - MATmin;
    elseif NORMCOL==2 % normalize lines
        MATmax = max(MAT,[],2);
        MATmin = min(MAT,[],2);
        MATrange = MATmax - MATmin;
    else
    ...
    if (iscell(page)), % if data is of type cell array
      ...
      if (~isempty(COLOR) && isnumeric(MAT)),
        for icol = 1:size(page,2),
          ...
          if NORMCOL ==1
            value = (page(ii,icol)-MATmin(icol))/MATrange(icol);
          elseif NORMCOL ==2
              value = (page(ii,icol)-MATmin(ii))/MATrange(ii);
          else
              value = (page(ii,icol)-MATmin)/MATrange;
          end
          color = dec2hex( floor(255*interp1((0:1/(size(COLOR,1)-1):1),COLOR,value)) );
